...
// });
// limit api calls for testing on low-bandwidth
for (let i = 0; i < 10; i++) {
  addressArray.push(real_data[i].address);
}
...
export default class Map extends React.Component {
  ...
  getCoordinates = async () => {
    ...
    for (const address of addressArray) {
      await fetch(
        `https://www.geogratis.gc.ca/services/geolocation/en/locate?q=${address}`
      )
        .then((response) => {
  ...
  createGeoJSON = async () => {
    ...
    // this.state.coordinates.forEach((coordinatePair) => {
    //   features.push({
    //     type: "Feature",
